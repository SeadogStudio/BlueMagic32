...
BlueMagicState::BlueMagicState() : _timecode("00:00:00:00"), _timecodeRaw(0), _transportMode(0), _aperture(0.0f), _focus(0.0f), _zoom(0), _iso(0), _shutter(0), _codec(0), _quality(0), _frameRate(0), _sensorFrameRate(0), _frameWidth(0), _frameHeight(0), _flags(0), _whiteBalance(0), _tint(0), _status(0) {
...
void BlueMagicState::settingsNotify(bool changed, uint8_t *data) {
    ...
    _settingsData = data; // Assumes data is valid and externally managed
}
...
void BlueMagicState::timecodeNotify(bool changed, uint8_t *data) {
    ...
    _timecodeData = data; // Assumes data is valid and externally managed
}
...
void BlueMagicState::statusNotify(bool changed, uint8_t *data) {
    ...
    _statusData = data; // Assumes data is valid and externally managed
}
...
void BlueMagicState::setTimecode(uint8_t H, uint8_t M, uint8_t S, uint8_t f) {
    ...
    _timecode = timecode; // Changed to character array
    _timecodeRaw = atoi(timecode + 0) * 1000000 + atoi(timecode + 3) * 10000 + atoi(timecode + 6) * 100 + atoi(timecode + 9);
}

String BlueMagicState::getTimecode() {
    return String(_timecode); // Returns a String version
}
